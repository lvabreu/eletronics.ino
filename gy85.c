...
const char* GY85_I2C::TAG = "GY85_I2C";

// --- Auxiliares ---
esp_err_t GY85_I2C::i2c_write_byte(i2c_master_dev_handle_t dev, uint8_t reg, uint8_t val) {
    uint8_t data[2] = {reg, val};
    return i2c_master_transmit(dev, data, 2, pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}

esp_err_t GY85_I2C::i2c_read_bytes(i2c_master_dev_handle_t dev, uint8_t reg, uint8_t *buf, size_t len) {
    esp_err_t ret = i2c_master_transmit_receive(dev, &reg, 1, buf, len, pdMS_TO_TICKS(I2C_TIMEOUT_MS));
    return ret;
}

// --- Inicialização ---
esp_err_t GY85_I2C::init(i2c_port_t port, gpio_num_t sda, gpio_num_t scl) {
